...
extern void one_player_layout();
extern void two_player_layout();
...
//------------------------------------------------------------------------------
void CGame::MainLoop(int pause)
{
	m_pSound->UpdateModule();
	if (pause==1 && m_MainCommand==MC_GAME) {
...
void CGame::TitleScreenLoop(void)
{
	...
	{
		if (m_pJoy2->fire) two_player_layout();
...
void CGame::EditName(JOYSTICK *pjoy, char *nptr)
{
	...
	if (key)
	{
			...
			(key==' ') )
		{
			...
			if (pjoy->up || pjoy->down)
				m_pSound->PlaySample(SND_XYLO);
		//else {
			if (pjoy->left)
			{
				m_HiOffset--;
				if (m_HiOffset<0) m_HiOffset = 3;
				pjoy->key = nptr[m_HiOffset];
				m_pSound->PlaySample(SND_XYLO);
				SDL_Delay(100);
			}
			if (pjoy->right)
